...
ADR-0008-Vertrag. Erfüllt **ACC-002** („Das Gebäude wird automatisch
als 3D-Modell dargestellt") und die sichtbare Hälfte von
**LH-FA-D3-002** (Scope-Herkunft: slice-010a, Roadmap-Drift-Tabelle
2026-06-11). In der Welle hinzugekommen: **slice-012**
(Eckenschluss endpunkt-verbundener Wände, LH-FA-WAL-006-Teilumfang) —
ausgelöst durch den Abnahme-Befund am ACC-002-Beleg.
...
- **Alle Closure-Trigger der Roadmap erfüllt:** drei Slices liegen in
  ...
  Der ACC-002-Beleg ist **vom Projektinhaber abgenommen** (Runde 2,
...
- **`make gates` grün** (frischer Lauf 2026-06-13 am aktuellen HEAD
  ...
  Regeln A–E (hexagonale Schichtung + Qt-Grenze ADR-0009),
...
- **Unabhängige Welle-Verifikation gelaufen** (Reviewer ≠ Autor, MR-006-
...
| Slice | Ergebnis |
|---|---|
| slice-011a | ADR-0009 „GUI-Framework-Bindung Qt 6 (Driving Adapter)" accepted (Pflichten a–f) + Spec-Operationalisierung „sichtbar" |
| slice-011b | Sichtbarer 3D-Viewer: Qt-6-Widgets-Adapter, Tessellation über `ViewModelPort` (kein OCC in der GUI), Szenen-Surrogat + display-freie AK-Tests, arch-check **Regel E** (Qt nur `adapters/ui/` + `main.cpp`), `make acc-002-beleg` (manueller Abnahme-Schritt, kein Gate); **ADR-0010** (Headless-GL via Xvfb/llvmpipe) entstand als Implementierungs-Befund |
| slice-012 | Eckenschluss endpunkt-verbundener Wände (LH-FA-WAL-006-Teilumfang): `model::Footprint` + Footprint-Hoheit im Kern (Grad-2-Miter mit `WALL_MITER_LIMIT`, total), `GeometryKernelPort` auf `extrudeFootprint`/`tessellateFootprint`, Mehr-Element-Update `WallGeometryChanged` (transaktional), 8 WAL-006-AK-Tests; ACC-002-Beleg ohne offene Außenecken regeneriert |
...
*sichtbare* Hälfte und schließt damit **ACC-002**, das in welle-1
bewusst offen blieb
...
alle drei Closure-Trigger-Slices real in `done/`; ACC-002 + sichtbare
Hälfte LH-FA-D3-002 erfüllt; ADR-0009/0010-Vertrag deckungsgleich mit
...
- **LOW-1 (terminologisch, nicht-blockierend):** ADR-0010 trägt im
  Titel/Bezug „**präzisiert** ADR-0009 (f)" statt der von
  [`AGENTS.md` §2.5](../../../../AGENTS.md) für Korrekturen genannten
  `Supersedes`-Form. Bewertung: ADR-0010 ändert ADR-0009 **nicht in
  der Substanz** (nur das Mechanik-Detail offscreen-QPA → Xvfb/llvmpipe);
  ADR-0009 bleibt `Accepted`/aktiv und reserviert „Supersedes-ADR"
...
2. **„Lösung schärft nie das Lastenheft": 3× — Regel-Kandidat.**
   slice-012 schärfte LH-FA-WAL-006 ausdrücklich nur über die
...
3. **„Der Abnahme-Schritt ist ein Sensor": 1× kategorisiert** (neuer
   Zähler, slice-012-Lerneintrag). Der ACC-002-Beleg (bewusst **kein**
   Gate, ADR-0009 (f)) fand die offenen Außenecken, die in keiner
...
4. **„Umgebungsabhängige Mechanik-Details vor ADR-Accept mit
   ...
   ADR-0009 (f) traf eine faktisch falsche Plattform-Aussage
   (offscreen-QPA trägt kein GL), entdeckt erst beim echten Lauf →
   Präzisierung ADR-0010.
...
6. **„ADR-Beziehungstyp ‚präzisiert' vs. ‚Supersedes' für
   ...
   ADR-0010 nutzt „präzisiert" für eine reine Mechanik-Detailkorrektur
...
7. **„Post-Commit-Schritte sind total": weiterhin 2×** (unverändert
   ...
   transaktionale Mehr-Element-Garantie sind bereits durch ADR-0008
...
Abnahme-Artefakt (offene Außenecken am ACC-002-Beleg). Das bestätigt
...
- **ACC-002 erfüllt** — der sichtbare 3D-Viewer ist geliefert und
