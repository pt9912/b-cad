...
| `make arch-check` | hexagonale Schichtung (Kern ohne Qt/OCC/SQLite/`adapters/`; kein Adapter→Adapter; OCC-`.hxx` nur in `adapters/geometry/`, Regel C; `sqlite3*` nur in `adapters/persistence/`, Regel D; Qt-Header nur in `adapters/ui/` + `src/main.cpp`, Regel E; dynamisches Laden nur in `adapters/plugin/` inkl. `plugins/`-Verbot, Regel P1; Import-Grenze `plugins/` + `src/plugin_api/`, Regel P2) | [ADR-0001](docs/plan/adr/0001-hexagonale-architektur.md), [ADR-0002](docs/plan/adr/0002-geometrie-kern-opencascade.md), [ADR-0003](docs/plan/adr/0003-persistenz-sqlite.md), [ADR-0009](docs/plan/adr/0009-gui-framework-qt6.md), [ADR-0017](docs/plan/adr/0017-plugin-api-abi.md) |
