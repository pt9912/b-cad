...
|---|---|---|
| `make docs-check` | interne Links, Anker, Inline-Code-Pfade + **Referenz-Richtung Spec→ADR + ADR↛Slice** (`matrix`/`ids`) + Span-/Host-Pfad-Hygiene; kein Pfad aus dem Repo, keine Abwärts-Referenz Spec → ADR, **kein Slice-Token im ADR-Körper** (no-downward, ab d-check v0.37.1) — via d-check (`d-check.mk`, digest-gepinnt v0.51.1, `.d-check.yml`; netzlos read-only Bind-Mount; Module links/anchors/codepaths/spans/hostpaths/matrix/ids/**planning**/**tracked** [Lifecycle + Fresh-Clone-Schutz; hermetisch/range-frei]) | [`MR-007`](conventions.md#mr-007--auflösung-von-mr-003-docs-check-via-d-check), [`MR-011`](conventions.md#mr-011--referenz-integritäts-gate-matrix-ids-spans-hostpaths), [`MR-014`](conventions.md), [`MR-017`](conventions.md), [`MR-018`](conventions.md) |
...
| `make golden-check` | **kein Gate** (nicht in `gates` → CI-Befehlsliste, Muster `make schema-check`; slice-044a): Byte-Drift der **Export-Golden** — die committeten `tests/adapters/golden/model.*` == `golden_gen`-Frisch-Ausgabe (byte-genau; zugleich Regen-Determinismus). Der eigentliche Byte-Vergleich läuft als GoogleTest `GoldenExport.*` **in `make test`/`gates`** | LH-Bindung [LH-FA-IO-001](../spec/lastenheft.md#lh-fa-io-001--ifc-import) … [LH-FA-IO-006](../spec/lastenheft.md#lh-fa-io-006), [ADR-0004](../docs/plan/adr/0004-toolchain-dependency-pinning.md) |
| `make golden-regen` | **kein Gate** (Muster `make schema-regen`; slice-044a): erzeugt die sechs Export-Golden neu (dedizierter `golden_gen` = geteilte `goldenModel()`-TU, **nicht** das Produktions-Binary; writable Mount) — nach bewusster Encoder-/Modell-Änderung, dann Diff-Review + `make golden-check` | [ADR-0004](../docs/plan/adr/0004-toolchain-dependency-pinning.md) |
...
| `make doc-immutable` | ADR-Immutabilität: der **Core** einer `Accepted`-ADR (ohne `## Geschichte`) unverändert über eine Range (d-check-Modul `vcs`, git-Diff, `d-check.mk`; `head-allow` erlaubt den `Superseded by`-Übergang) — macht [`AGENTS.md` §2.5](../AGENTS.md) computational; **kein Gate** (nicht in `gates` → CI-Befehlsliste, Muster `make doc-commits`) | [MR-016](conventions.md) |
