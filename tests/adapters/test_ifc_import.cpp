...
TEST(IfcImportIntegration, ExchangeServiceImportsThroughRealAdapter) {
    ...
    const ExchangeService service({{ExchangeFormat::Ifc, &adapter}}, {{ExchangeFormat::Ifc, &exporter}});  // Driven-Port-Injektion (main-Muster)
...
TEST(IfcImportIntegration, ExchangeServicePropagatesEIo003) {
    ...
    const IfcImportAdapter adapter;
    const bcad::adapters::io::IfcExportAdapter exporter;
    const ExchangeService service({{ExchangeFormat::Ifc, &adapter}}, {{ExchangeFormat::Ifc, &exporter}});
